...
import com.liu.redisTest.redisUntll.JedisUtil;
...
public class RedisTest {
	...
	@Test public void testString() throws InterruptedException
	  {
	      ...
	      System.out.println("在key3后面加入值："+jedis.append("key3", "End"));//后面加值？？？
	      ...
	      System.out.println(jedis.setnx("key1", "value1"));//"SET if Not eXists".有原值的话不能覆盖。没有这个键值对的话正常增加。
	      ...
	      System.out.println(jedis.setex("key3", 2, "value3"));//设置有效时间2秒。
	...
	public void testKey() throws InterruptedException
	   {
	       ...
	       Set<String> keys = jedis.keys("*");//系统中所有的键如下
	       System.out.println(keys);
	       System.out.println("删除键password:"+jedis.del("password"));
	       System.out.println("判断键password是否存在："+jedis.exists("password"));
	       System.out.println("设置键username的过期时间为5s:"+jedis.expire("username", 5));
	       TimeUnit.SECONDS.sleep(2);
	       System.out.println("查看键username的剩余生存时间："+jedis.ttl("username"));//查看键username的剩余生存时间
	       System.out.println("移除键username的生存时间："+jedis.persist("username"));//移除键username的生存时间：
	...
	@Test public void testList()//列表
	{
	    ...
	    //lpush命令在list头插入一个元素，使用RPUSH命令在list的尾插入一个元素
	...
	@Test public void testSet()//集合（Set）
	  {
	      jedis.flushDB();
